...
function VRNavigation:StartFollowingPath(newPath)
	...
	local currentPoints = currentPath:GetPointCoordinates()
	...
	if humanoid and humanoid.Torso and #currentPoints >= 1 then
		...
		local expectedTimeToNextPoint = dist / humanoid.WalkSpeed
	end
...
function VRNavigation:GoToPoint(point)
	...
	local timeReachedLastPoint = tick()
	local expectedTimeToNextPoint = estimatedTimeRemaining
...
function VRNavigation:OnHeartbeat(dt)
	...
	if self.moving and self.currentPoints then
		...
		if moveDist < POINT_REACHED_THRESHOLD then
			...
			local currentPointIdx = self.currentPointIdx + 1
			...
			if #self.currentPoints == 0 then
				...
				local expectedTimeToNextPoint = distanceToGoal / humanoid.WalkSpeed
				local timeReachedLastPoint = tick()
			end
		else
			local ignoreTable = {
